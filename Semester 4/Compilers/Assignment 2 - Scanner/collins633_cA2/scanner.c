...
Token createStringToken(int code, char *attribute);
...
Token malpar_next_token(Buffer * sc_buf)
{
	...
	static int forward = 0;  /* current input char offset */ 
...
Token aa_func02(char *lexeme){
	...
	if (lexeme[strlen(lexeme)-1] == '#'){
		...
		lexeme[VID_LEN-1] = '#';
   		return createStringToken(SVID_T, lexeme);
	}

	/*Lexeme is an AVID*/
	return createStringToken(AVID_T, lexeme);
}
...
/*
 * Purpose: To ease the creation of tokens with string attributes
 * Author: William Collins (040652633)
 * History/Versions: 1.0
 * Parameters:	code - the code for the token
 					attribute - the string attribute for the token
 * Return value: A token with your specified values
 */
Token createStringToken(int code, char *attribute){
	Token t;
	...
	t.code = code;
	for (i = 0; i < VID_LEN && attribute[i] != '\0'; i++){
   		t.attribute.vid_lex[i] = attribute[i];
	}
