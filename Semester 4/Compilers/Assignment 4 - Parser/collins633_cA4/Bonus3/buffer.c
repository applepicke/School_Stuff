...
Buffer *ca_addc(Buffer *pB, char symbol)
{
	...
	if ( (sizeof(char) * pB->addc_offset) >= pB->capacity)
	{
 ...
 * Return value: -2 on error, otherwise returns the mode 
*/
...
Buffer *ca_pack( Buffer *pB )
{
   ...
   /*Make sure that we don't have a weird offset*/
	if (pB->addc_offset > pB->capacity || pB->addc_offset < 0 || pB->addc_offset == INT_MAX)
	{
		return NULL;
	}
...
/*
 * Purpose: copies a file of data into the buffer
 * Author: William Collins (040652633)
 * History/Versions: 1.0
