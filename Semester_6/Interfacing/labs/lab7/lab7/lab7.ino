...
void setup() {
  ...
  pinMode( AUDIO_TONE, OUTPUT );
  
  pinMode( LIGHT_IN, INPUT );
  pinMode( TIN_OUT, OUTPUT );
  pinMode( TIN_IN, INPUT );
  
  Serial.begin( 9400 );
}
CapSense sensor = CapSense( TIN_OUT, TIN_IN );

void loop() {
  long note = sensor.capSense(30) / 4;
  long volume = analogRead( LIGHT_IN ) * 1600L / 1023L / 100;
  //Serial.println( volume, DEC );
  playNewSound( note, volume );
}
...
void playNewSound( int note, int volume ) {  
  noTone( AUDIO_TONE );
  int mask = 0x08;
